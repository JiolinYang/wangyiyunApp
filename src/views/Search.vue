...
export default {
  data() {
    return {
      keyWorldList: [],//关键字保存在里面
      searchKey: "",  //输入的值 保存在这里 然后放在KeyWorldList这个数组里面
      searchList: [],//搜索列表
    };
  },
  mounted() {
    //如果有值就为本身 如果没有值就空 给个默认的值
  ...
  methods: {
    enterKey: async function () {
      if (this.searchKey !== "") {
        //使用Unshit 将searchKey 添加进 keyWorldList 用unshift 不同push是新的元素可以在第一位
        this.keyWorldList.unshift(this.searchKey);
        //   去重 用set 返回一个新的数组 这个数组是没有重复的
    ...
    delHistory: function () {
      //删除localStorage 和keyWorldList里面的
    ...
    updateIndex:function(item){
      //将图片改为item里的值
        ...
        //将item传入到播放列表中 穿进去后需要播放 所以需要调用updatePlayListIndex
