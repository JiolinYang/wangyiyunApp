<template>
  ...
  <div class="musicLyric" ref="musicLyric" v-show="isLyricShow">
   <!-- 如果当前时间大于歌词自身的时间 并且小于下一句歌词的时间才能显示样式 -->
    <p
      v-for="item in lyric"
      :key="item"
      :class="{
  ...
  <div class="detailFooter">
    ...
    <div class="footerContent">
      <!-- 最小值為0 最大值是duration是动态的 使用动态双向绑定 -->
...
export default {
    ...
    lyric: function () {
      ...
      if (this.lyricList.lyric) {
        arr = this.lyricList.lyric.split(/[(\r\n)\r\n]+/).map((item, i) => {
          ...
          //以对象的形式返回出去拿到歌词  时间放在对象里面需要的时候再拿出来
          return { min, sec, mill, lrc, time };
        });
        arr.forEach((item, i) => {
          //当到最后一句就不需要拿到歌词的时间了 如果不是就等于歌词下一局的时间
  ...
  methods: {
    //回到home页面 updateDetailShow再次进入还是显示磁盘磁针
    backHome: function () {
  ...
  watch: {
    currentTime: function (newValue) {
      ...
      // console.log([p]);
      //如果到父级的顶部距离大于300  当前的歌词就-300 保持居中
      //scrollTop为滚动条距离
      if (p) {
        if (p.offsetTop > 300) {
